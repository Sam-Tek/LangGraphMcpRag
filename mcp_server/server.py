...
async def health_check(request: Request) -> JSONResponse:
    """HTTP health check used by dependent services to gate startup."""
...
def get_collection():
    ...
    Double-checked locking guarantees thread-safety without acquiring the lock
    on every call once the collection is initialized.
    """
    global _chroma_client, _collection
    if _collection is None:
        with _collection_lock:
            if _collection is None:  # re-vérification après acquisition du verrou
...
def search_knowledge_base(query: str, n_results: int = 4) -> str:
    ...
    for i, (doc, meta, dist) in enumerate(zip(docs, metas, distances), 1):
        ...
        # L2 distance pour des vecteurs normalisés ∈ [0, 2] → diviser par 2 pour obtenir [0, 1]
...
if __name__ == "__main__":
    _wait_for_chroma()
    _seed_data()
    # Pré-chauffe réelle : force le chargement du modèle d'embedding en mémoire
    # avant la première requête client, évitant une latence initiale de ~2 secondes.
